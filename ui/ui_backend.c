...
void SpinControl_Draw (menulist_s *s)
{
	...
	if (s->curvalue < 0)
	{
		//fix crash when r_mode is less than zero.
		Menu_DrawString (s->generic.x + s->generic.parent->x + RCOLUMN_OFFSET, s->generic.y + s->generic.parent->y, "???", alpha);
	}
	else if (!strchr(s->itemnames[s->curvalue], '\n') && s->curvalue >= 0)
	{
		Menu_DrawString (s->generic.x + s->generic.parent->x + RCOLUMN_OFFSET, s->generic.y + s->generic.parent->y, s->itemnames[s->curvalue] , alpha);
	}	
	else
	{
...
void checkbox_Draw( menulist_s *s  )
{
	...
	if (buffer)
	{
		...
		//BC fix rendering of checkbox ui element 10-29-2018
